...
if __name__=="__main__":
    sty_pic = tf.placeholder('float', [None, None, None, 3])
    cot_pic = tf.placeholder('float', [None, None, None, 3])
    para = np.load("D:/pythprop/data_for_use/vgg16.npy",encoding='latin1').item()
    batch_size = 4
    style_batch = np.zeros([batch_size, 256, 256, 3])
    style_pic = np.array(Image.open("D:/pythprop/pic_for_use/style_wave.jpg"))[np.newaxis,:,:,:]
    style_batch = np.concatenate([style_pic,style_pic,style_pic,style_pic],axis=0)
    list_imgs = os.listdir("D:/project/ILSVRC2012_img_val")
    y_hat = style_training_net(cot_pic)
    f_y_hat = VGG(y_hat, para)
    f_sty = VGG(sty_pic, para)
    f_cot = VGG(cot_pic, para)
    l_content = con_loss(f_cot, f_y_hat)
    l_style = sty_loss(f_sty, f_y_hat)
    loss = l_content * 5 + l_style * 280
    train_step = tf.train.AdamOptimizer(0.001).minimize(loss)
    # batch = 3
    # epoch = int(list_imgs.__len__())
    path = "D:/project/ILSVRC2012_img_val/"
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        saver = tf.train.Saver()
        for i in range(40000):
            batch = read_data(path, batch_size)
            sess.run(train_step, feed_dict={cot_pic: batch, sty_pic: style_batch})
            if i % 10 == 0:
                [LOSS, L_CON, L_STY] = sess.run([loss, l_content, l_style], feed_dict={cot_pic: batch, sty_pic: style_batch})
                print("Step: %d, Total loss: %e, Content loss: %e, Style loss: %e"%(i, LOSS, L_CON, L_STY))
            if i % 100 == 0:
                img = sess.run(y_hat, feed_dict={cot_pic: batch, sty_pic: style_batch})
                img = np.concatenate((batch[0, :, :, :], img[0, :, :, :]), axis=1)
